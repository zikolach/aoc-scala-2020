object Day02 {
  ...
  def ex1(): Unit = {
    ...
    val result = input.count {
      case re(min, max, char, password) =>
        val n = password.count(_ == char.head)
        val b = (n >= min.toInt) && (n <= max.toInt)
        println(min, max, char, password, b)
        b
    }
    println(result)
  }
  ...
  def ex2(): Unit = {
    val re = "^(\\d+)-(\\d+) (\\w):\\s(\\w+)$".r
    val result = input.count {
      case re(p1, p2, char, password) =>
        val a = password.charAt(p1.toInt - 1) == char.head
        val b = password.charAt(p2.toInt - 1) == char.head
        val res = (a || b) && !(a && b)
        println(p1, p2, char, password, res)
        res
    }
    println(result)
  }
